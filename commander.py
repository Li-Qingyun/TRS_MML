...
# adjust depths of the encoder and decoder

class EDdepth:
    def __init__(self, encoder_depth, decoder_depth):
        self.encoder = encoder_depth
        self.decoder = decoder_depth
        self.message = f'Encoder: {encoder_depth}    Decoder: {decoder_depth}'
...
warmup_steps = 20
settings_list = []
for i in ['', '', '']:  # Encoder depth
    for j in ['', '', '']:  # Decoder depth
        settings_list.append(EDdepth(i, j))
...
for exp_inx, settings in enumerate(settings_list):
    ...
    command = "python run.py config=TRS_MML/configs/single_task/indian_single.yaml " \
              ...
              f"indian_single_lr={lr}_{lr_backbone}_EDdepth={settings.encoder}_{settings.decoder} " \
              f"model_config.trs.base_args.enc_layers={settings.encoder} " \
              f"model_config.trs.base_args.dec_layers={settings.decoder} " \
              "env.data_dir=C:/Users/LQY/Desktop " \
              "checkpoint.max_to_keep=1 " \
              f"optimizer.params.lr={lr} " \
              f"model_config.trs.base_args.lr_backbone={lr_backbone} " \
              f"scheduler.params.num_warmup_steps={warmup_steps} " \
              "training.log_interval=2 " \
              f"training.seed={seed} " \    
              "training.evaluation_interval=2 "    # evaluation frequency (2 steps equal to 1 epoch)
